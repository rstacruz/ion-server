...
task :'test' => :'redis:start' do
  ...
  Dir['test/unit/*_test.rb'].each { |f| load f }
end
...
desc "Starts the server in development mode."
task :start => :'redis:start' do
  info "Starting ion-server..."
  # Should probably be redis_url(2) in the future, when it's independent?
  system "env REDIS_URL=#{redis_url(0)} shotgun -s thin www/config.ru -p 8080"
end
